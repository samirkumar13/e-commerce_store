...
| referredBy | String? | Referral code used at signup; cleared after referrer bonus fires |
...
### Admin — `/api/admin/returns` (auth required, `orders` permission)
| Method | Path | Description |
|---|---|---|
...
| GET | `/history` | Get full wallet transaction history |
...
- **Checkout flow**:
  ...
  3. Backend creates `PENDING_PAYMENT` order, debits wallet if points used, returns PhonePe payment URL
...
- **Order cancellation** — user can cancel from account page when order is still in `PENDING` or `PROCESSING`
...
**Earning points:**
- Points are awarded when PhonePe payment is confirmed (order moves to PAID)
...
- Wallet debit happens before payment initiation; order records the deducted amount
- Recorded as `DEBIT_ORDER` wallet transaction
...
| `CREDIT_ADMIN` | Manual credit by admin |
...
- Full transaction history per user visible in admin panel
...
- API routes enforce the same permission checks server-side (not just UI-level)
...
- Integration uses PhonePe's hosted checkout (redirect flow):
  1. Backend calls PhonePe API to create a payment session
  2. Returns a redirect URL to the frontend
  3. User completes payment on PhonePe's page
  4. PhonePe redirects back to the app
  5. Frontend polls the backend status endpoint
  6. Backend verifies payment with PhonePe API, updates order, awards points
- Orders that are never paid remain in `PENDING_PAYMENT` status indefinitely
- `pointsToRedeem` is passed at checkout initiation; wallet is debited before payment redirect
...
High-effort review performed on 2026-06-15 covering all uncommitted changes (wallet, variants, returns, staff roles, flash sale, coupon per-user limit). 9 findings; 8 fixed, 1 confirmed as already correct.
