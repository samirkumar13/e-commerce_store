...
// Product Management
export const getAllProducts = () => prisma.product.findMany({ include: { category: true }, orderBy: { createdAt: 'desc' } });
export const getLowStockProducts = (threshold: number) => prisma.product.findMany({
    where: { stock: { lte: threshold } },
    include: { category: true },
    orderBy: { stock: 'asc' },
});
export const createProduct = (data: Prisma.ProductCreateInput) => prisma.product.create({ data });
export const updateProduct = (id: string, data: Prisma.ProductUpdateInput) => prisma.product.update({ where: { id }, data });
...
export const updateSlide = (id: string, data: Prisma.HomeSlideUpdateInput) => prisma.homeSlide.update({ where: { id }, data });
